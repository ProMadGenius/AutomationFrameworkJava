...
public class GenericUtilities {
	...
	//Method For converting json to list of strings
	public static List<SearchWord> readJsonFileToList(String filePath) {
		try {
			Reader file = Files.newBufferedReader(Paths.get(filePath));
			Gson gson = new Gson();
			//T object = gson.fromJson(file, classOfT);
			List<SearchWord> list = gson.fromJson(file, new TypeToken<List<SearchWord>>(){}.getType());
			file.close();
			return list;
		} catch (IOException e) {
			throw new RuntimeException(String.format("Error when reading json file, %s", e.getMessage()));
